...
- **npm 发布（用户拍板「帮我npm 发版」）**：`dsh-api-client@0.2.0`（e4c9ce0，semver minor）已发布 registry latest；发布后实测：干净目录 `npm install dsh-api-client@0.2.0` 成功（8 包），dist 三件齐（client.js/index.js/index.js.map），exports 完整，**发布物 client bundle grep undici = 0**（F-01 防线在发布面复验）；README Roadmap 同步（V0.2=当前 P0，原导入类条目顺延 V0.3 并纳入 R-09/tab identity/Undo 遗留）。
- **插件市场收录 PR（用户拍板「提交到 DSH 插件市场」）**：仓库加 `dsh-plugin` 等 5 个 topic → fork awesome-dsh-plugin → 单文件条目 `data/plugins/Beatther-c__dsh-api-client.yml`（category=tools，en/zh 描述对照 dsh-mysql 模板，ruby YAML 解析校验过）→ **PR [awesome-dsh-plugin#4819](https://github.com/awesome-dsh-plugin/awesome-dsh-plugin/pull/4819)**（双语推荐文案：人机双轨同核/安全设计/589 测试与真实运行态验收证据/安装命令/投稿自查清单全勾）；mergeable 无冲突，CI 排队中（仓库创建满 1 天等硬性项均已预核）。
